...
class Api {
  ...
  static Future<void> _saveDisk() async {
    try {
      ...
      // حفظ كامل المحتوى (لا قص) ليظهر كل الأفلام/المسلسلات/القنوات
      await p.setString('tv_m', jsonEncode(movies));
      await p.setString('tv_s', jsonEncode(series));
      await p.setString('tv_l', jsonEncode(live));
